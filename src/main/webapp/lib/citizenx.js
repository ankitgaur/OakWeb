	...
	$.ajax({
		...
		success : function(result) {
			...
			var chart = new google.visualization.BarChart(document
					.getElementById('incident_canvas'));
	...
	$.ajax({
		...
		success : function(result) {
			...
			var chart = new google.visualization.ColumnChart(document
					.getElementById('nigeria_govts_canvas'));
...
function drawStateChart() {
	$.ajax({
		...
		success : function(result) {
			...
			var chart = new google.visualization.ColumnChart(document
					.getElementById('nigeria_states_canvas'));
...
function drawAccidentChart() {
	$.ajax({
		...
		success : function(result) {
			...
			var chart = new google.visualization.ColumnChart(document
					.getElementById('accident_canvas'));
...
function drawHospitalChart() {
	$.ajax({
		...
		success : function(result) {
			...
			var chart = new google.visualization.ColumnChart(document
					.getElementById('hospital_canvas'));
...
function drawTransportChart() {
	$.ajax({
		...
		success : function(result) {
			...
			var chart = new google.visualization.ColumnChart(document
					.getElementById('transport_canvas'));
	...
	$
			.ajax({
				...
				success : function(result) {
					...
					var options = {};
					options['region'] = 'NG';
					options['colors'] = [ 0xADD8E6, 0x27408B ]; // yellow-orange-red
																// colors
					options['dataMode'] = 'regions';
					options['resolution'] = 'provinces';
					options['width'] = $('#nigeria_canvas').parent().width() - 10;
					...
					google.visualization.events
							.addListener(
									...
									function() {
										...
										if (selection.length) {
											// alert(data.getValue(selection[0].row,
											// 0));
											// drawMap();
											window.location = 'http://www.ipledge2nigeria.com/state.html#'
													+ data
															.getValue(
																	selection[0].row,
																	0);
